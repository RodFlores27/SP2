...
`penciled` — `ACTIVE_PENCIL_STATUSES`.
...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `BOOKING_MIN_LEAD_TIME` | New bookings are blocked when selected request type lead time is not met (equipment in-house: 2 days, equipment loan: 7 days, room: 7 days) |
...
| P-08  | `contested`                         | `displaced`                                  | Defender expiry-boundary loss during contention                  | Cron       | Defender pencil `expiryAt <= now` while acting as defender                                            | Defender terminal `displaced`; challenger → `penciled` if not terminal                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| P-09  | `penciled`                          | `displaced`                                  | `applyChallengerWins` (defender branch)                          | Cron       | `resolveDueContentionEpisodes` past `deadlineAt`                                                      | Defender `displaced` remark set                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| ----- | ------------------------- | ----------------------------------------------------------------- | ------------------------------------ |
| G-00  | `POST /bookings`          | Start below request-type lead time requirement                    | 400 `BOOKING_MIN_LEAD_TIME`          |
...
Separate daily job: expiry **warnings** email (`notifyBookingExpiringSoon`) for pencils in `penciled`.
...
| **Booking controller**   | **`GET /bookings/availability`**: optional **`contentionChallenger`** per row (open episode challenger). Create-time own-pencil overlap checks now include `on_hold` (same-user) in addition to `penciled`, and firm `confirmOverlapOwn` auto-cancels both statuses.                                                                                                                                                                                                                                                                                                                                                             |
...
| **My Bookings / badges** | **`BookingStatusBadge`**: `queued` / `displaced` plus role badges (`Challenger` / `Defender`); **`ActiveBookingCard`**: warning/alert notices (challenger, defender, firm-pending overlap, pencil-on-hold) follow a unified integrated-card format (visible summary + collapsed **View details**); challenger convert action is disabled; **`PastBookingRow`**: displaced terminal + rebook messaging; **`BookingToolbar`**: status filters include `queued` / `displaced`.                                                                                                                                                     |
...
| My Bookings           | Queued/displaced statuses are surfaced in badges and filters; role badges now include both Challenger and Defender where applicable. Active-card warning notices (challenger, defender, firm-pending overlap, pencil-on-hold) use a unified integrated-card pattern: concise visible summary + collapsed **View details**. Past rows retain displaced terminal + rebook messaging (`canRebook` / displacing firm still pending). |
