...
`cancelled`, `denied`, `expired`, `displaced` — see `TERMINAL_STATUSES` in `server/models/booking.js`.
...
|----------------|---------|
| `BOOKING_LOCK_WINDOW` | Create (and firm cancel per controller) blocked when `hoursUntilStart(start) <= 24` |
...
| P-02 | — | `penciled` | Same, but `openEpisode` after create | User | Foreign pencils overlap + `confirmContention`; defender was `penciled` | Defender **same row** → `contested` (P-03); challenger stays `penciled` |
...
**`expired` on firm:** not set in the snippets reviewed; mark **TBD** if you have another job.
...
| 4 | Bulk pencil `expired` | `penciled`/`queued` with `expiryAt <= now` |
