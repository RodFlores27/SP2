...
/**
 * Hybrid calendar showcase seed:
 * keeps a multi-day spread for realistic month/week screenshots and also
 * adds a single-day dense cluster for one-day "all states visible" screenshots.
 * It still covers
 * every type the /bookings/availability view can return
 * (non-terminal rows only: cancelled/denied/expired/displaced/completed
...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    const now = new Date();
    // Place showcase after initial-data demo windows (latest initial demo day is day 9).
    // Spread groups across multiple days so calendar screenshots show variety over time.
    const baseOffset = 16;
    const lamDay = baseOffset;
    const autoDay = baseOffset + 2;
    const growthDay = baseOffset + 4;
    const cultureDay = baseOffset + 6;
    const prepDay = baseOffset + 8;
    const singleDay = baseOffset + 11;
    ...
    const lam1Start = at(lamDay, 6, 0);
    const lam1End = at(lamDay, 7, 30);
    await insertBooking({
      userId: studentId,
    ...
    const lamFirmApprStart = at(lamDay, 8, 0);
    const lamFirmApprEnd = at(lamDay, 9, 0);
    await insertBooking({
      userId: staffId,
      resourceType: 'equipment',
      resourceId: laminarId,
    ...
    const lamFirmPendStart = at(lamDay, 9, 30);
    const lamFirmPendEnd = at(lamDay, 10, 30);
    await insertBooking({
      userId: studentId,
      resourceType: 'equipment',
      resourceId: laminarId,
    ...
    const aDef1s = at(autoDay, 11, 0);
    const aDef1e = at(autoDay, 15, 0);
    const aCh1s = at(autoDay, 11, 30);
    const aCh1e = at(autoDay, 13, 0);
    const aDef2s = at(autoDay, 16, 0);
    const aDef2e = at(autoDay, 20, 0);
    const aCh2s = at(autoDay, 16, 30);
    const aCh2e = at(autoDay, 18, 0);
    ...
    const grFirmS = at(growthDay, 10, 0);
    const grFirmE = at(growthDay, 13, 0);
    await insertBooking({
      userId: studentId,
      resourceType: 'equipment',
      resourceId: growthId,
    ...
    const cuApprS = at(cultureDay, 16, 0);
    const cuApprE = at(cultureDay, 18, 0);
    await insertBooking({
      userId: staffId,
      resourceType: 'room',
    ...
    const cuFreeS = at(cultureDay, 19, 0);
    const cuFreeE = at(cultureDay, 20, 0);
    await insertBooking({
      userId: researcher1Id,
      resourceType: 'room',
      resourceId: cultureAId,
    ...
    const prFirmS = at(prepDay, 7, 0);
    const prFirmE = at(prepDay, 10, 0);
    await insertBooking({
      userId: studentId,
      resourceType: 'room',
    ...
    const prFreeS = at(prepDay, 20, 0);
    const prFreeE = at(prepDay, 21, 30);
    await insertBooking({
      userId: researcher1Id,
      resourceType: 'room',
      resourceId: prepRoomId,
    ...
    // --- Single-day dense cluster: all calendar-visible states on one date ---
    const sdNowMinus5h = new Date(now.getTime() - 5 * 60 * 60 * 1000);
    const sdNowMinus4h = new Date(now.getTime() - 4 * 60 * 60 * 1000);
    const sdNowMinus3h = new Date(now.getTime() - 3 * 60 * 60 * 1000);
    const sdNowMinus2h = new Date(now.getTime() - 2 * 60 * 60 * 1000);

    // free pencil
    const sdPencilStart = at(singleDay, 6, 30);
    const sdPencilEnd = at(singleDay, 7, 30);
    await insertBooking({
      userId: studentId,
      resourceType: 'equipment',
      resourceId: laminarId,
      bookingType: 'pencil',
      status: 'penciled',
      startTime: sdPencilStart,
      endTime: sdPencilEnd,
      purpose: 'SHOWCASE:SINGLEDAY: free pencil',
      authorizationDocUrl: null,
      equipmentRequestType: 'in_house',
      approvedByUserId: null,
      approvedAt: null,
      expiryAt: computePencilExpiryAt(sdNowMinus5h, sdPencilStart),
      createdAt: sdNowMinus5h,
      updatedAt: sdNowMinus5h,
    });
    ...
    await insertBooking({
      ...
      bookingType: 'firm',
      status: 'approved',
      startTime: at(singleDay, 8, 0),
      endTime: at(singleDay, 9, 0),
      purpose: 'SHOWCASE:SINGLEDAY: firm approved',
      authorizationDocUrl: 'https://res.cloudinary.com/demo/sample.pdf',
      equipmentRequestType: 'in_house',
      approvedByUserId: adminId,
      approvedAt: sdNowMinus2h,
      expiryAt: null,
      createdAt: now,
      updatedAt: now,
    });
    ...
    // pending firm
    const sdPendingStart = at(singleDay, 9, 30);
    const sdPendingEnd = at(singleDay, 11, 0);
    await insertBooking({
      userId: researcher1Id,
      resourceType: 'equipment',
      resourceId: growthId,
      bookingType: 'firm',
      status: 'pending_approval',
      startTime: sdPendingStart,
      endTime: sdPendingEnd,
      purpose: 'SHOWCASE:SINGLEDAY: firm pending',
      authorizationDocUrl: 'https://res.cloudinary.com/demo/sample.pdf',
      equipmentRequestType: 'loan',
      loanReason: 'SHOWCASE:SINGLEDAY: off-site run',
      loanWorkflowNote: 'SHOWCASE:SINGLEDAY: pending approval sample',
      loanTransportPlan: 'SHOWCASE:SINGLEDAY: protected transport',
      approvedByUserId: null,
      approvedAt: null,
      expiryAt: null,
      createdAt: now,
    ...
    // on_hold pencil overlapping the pending firm
    await insertBooking({
      userId: researcher2Id,
      resourceType: 'equipment',
      resourceId: growthId,
      bookingType: 'pencil',
      status: 'on_hold',
      startTime: at(singleDay, 9, 45),
      endTime: at(singleDay, 10, 45),
      purpose: 'SHOWCASE:SINGLEDAY: on-hold pencil',
      authorizationDocUrl: null,
      equipmentRequestType: 'in_house',
      approvedByUserId: null,
      approvedAt: null,
      expiryAt: computePencilExpiryAt(sdNowMinus4h, at(singleDay, 9, 45)),
      createdAt: sdNowMinus4h,
      updatedAt: sdNowMinus4h,
    });

    // 1v1 contention pair (defender + challenger)
    const sdDefStart = at(singleDay, 14, 0);
    const sdDefEnd = at(singleDay, 16, 0);
    const sdDefExpiry = computePencilExpiryAt(sdNowMinus3h, sdDefStart);
    const sdDeadline = computeContentionDeadline(sdNowMinus3h, sdDefStart, sdDefExpiry);

    const sdDefId = await insertBooking({
      userId: staffId,
      resourceType: 'room',
      resourceId: prepRoomId,
      bookingType: 'pencil',
      status: 'penciled',
      startTime: sdDefStart,
      endTime: sdDefEnd,
      purpose: 'SHOWCASE:SINGLEDAY: contention defender',
      authorizationDocUrl: null,
      roomParticipantCount: 8,
      roomEquipmentNeeds: 'Marker set',
      roomSetupRequirements: 'Classroom layout',
      roomProgramDetails: 'SHOWCASE:SINGLEDAY: contention pair',
      approvedByUserId: null,
      approvedAt: null,
      expiryAt: sdDefExpiry,
      contentionRole: 'defender',
      contentionDeadlineAt: sdDeadline,
      challengingBookingId: null,
      createdAt: sdNowMinus3h,
      updatedAt: sdNowMinus3h,
    });
    ...
    await insertBooking({
      ...
      bookingType: 'pencil',
      status: 'penciled',
      startTime: at(singleDay, 14, 30),
      endTime: at(singleDay, 15, 30),
      purpose: 'SHOWCASE:SINGLEDAY: contention challenger',
      authorizationDocUrl: null,
      roomParticipantCount: 4,
      roomEquipmentNeeds: 'None',
      roomSetupRequirements: 'No special setup',
      roomProgramDetails: 'SHOWCASE:SINGLEDAY: challenger sample',
      approvedByUserId: null,
      approvedAt: null,
      expiryAt: computePencilExpiryAt(sdNowMinus2h, at(singleDay, 14, 30)),
      contentionRole: 'challenger',
      contentionDeadlineAt: null,
      challengingBookingId: sdDefId,
      createdAt: sdNowMinus2h,
      updatedAt: sdNowMinus2h,
    });
  },
