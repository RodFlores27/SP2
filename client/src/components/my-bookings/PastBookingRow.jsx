    ...
    <div className="flex flex-col sm:flex-row sm:items-start gap-2 py-3 px-4 rounded-lg border border-border bg-card opacity-70 hover:opacity-90 transition-opacity">
      <div className="space-y-1 min-w-0 flex-1">
        {rebookBlockedByActiveFirm && (
          <div className="mb-2 rounded-md border border-up-gold/30 bg-accent px-3 py-2 text-xs text-accent-foreground">
        ...
        {previousAttempts.length > 0 && (
          <div className="rounded-md border border-up-gold/30 bg-accent px-2.5 py-2 text-xs">
            <button
              type="button"
            ...
            {showPreviousAttempts && (
              <div className="mt-1.5 space-y-1.5">
                {previousAttempts.map((attempt) => (
                  ...
                  {!!attempt.staffRemark && (
                    <p className="text-accent-foreground/75">{attempt.staffRemark}</p>
                  )}
                  </div>
                ))}
              </div>
